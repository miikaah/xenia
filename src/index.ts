...
const getFormattedDate = (date: Date) => {
  return `${date.getFullYear()}${padLeft(date.getMonth() + 1)}${padLeft(date.getDate())}${padLeft(date.getHours())}${padLeft(date.getSeconds())}`
}
...
const getFileSize = async (directoryPath: string, totalFilesize: number): Promise<number> => {
...
const compressDirectory = async (directoryPath: string, outputFilePath: string, res: Response) => {
  console.log("Requested", directoryPath, "for archival")
  const files = await fs.readdir(directoryPath);

  let totalFilesize = await getFileSize(directoryPath, 0);
  console.log("Total size of the directory", humanizeSize(totalFilesize), "bytes");

  if (totalFilesize > 3_000_000_000) {
    res.status(401).json({ error: `File size of ${humanizeSize(totalFilesize)} is too large` });
    return;
  }
  ...
  const outputFilename = `${path.join(tempDirPath, `${outputFilePath}.${timestamp}.${totalFilesize}`)}.zip`;
  const outputFilenameForClient = `${path.join(tempDirPath, outputFilePath)}.zip`;
...
const start = async () => {
  ...
  await Promise.all(
    directories.map(async (directory) => {
      ...
      dirs.push({ path: directory, name, stat, isDirectory, isLeafNode: false });
    })
  )
